...
def generate_random_data(n=69):
    assert n >= 27, "Need enough space to insert 3 high values 8 apart"

    # Step 1: Choose 3 indices for values >80 that are at least 8 apart
    possible_positions = list(range(0, n))
    valid_triplets = []

    for i in range(0, n - 16):  # i + 8 + 8 <= n-1
        for j in range(i + 8, n - 8):
            for k in range(j + 8, n):
                valid_triplets.append((i, j, k))

    chosen_triplet = random.choice(valid_triplets)
    high_positions = set(chosen_triplet)

    # Step 2: Generate values with controlled difference and insert highs
    ...
    for i in range(1, n):
        if i in high_positions:
            ...
            # Avoid >80 unless we're at one of the high positions
            if value > 80:
