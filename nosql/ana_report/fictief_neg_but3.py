...
def generate_random_data():
    ...
    # Last 8 values: mainly red (< -7% change) or orange (-7% to 0% change), non-zero
    last_value = data[-1] if data else 50  # Start with the last of the first 61 or a default
    for _ in range(8):  # Generate Q62–Q69
...
def generate_plot_json(data):
    ...
    fig.update_layout(title="Random Data Bar Chart (Last 8 Mainly Red or Orange, Non-Zero)")
            ...
            <script>
                ...
                async function regeneratePlot() {{
                    try {{
                        ...
                        console.log('Last 8 values:', newData.data[0].y.slice(-8));
