...
def generate_random_data():
    ...
    for i in range(7):  # Generate Q62–Q68
        if np.random.choice([True, False], p=[0.75, 0.25]):  # Red: 75% chance
            # Ensure new_value < last_value * 0.93 for < -7% change
            max_val = int(last_value * 0.93)
            new_value = np.random.randint(0, max_val) if max_val > 0 else 0
        else:  # Orange: 25% chance
            min_val = int(last_value * 0.93)
            new_value = np.random.randint(max(min_val, 0), last_value + 1)
        data.append(new_value)
        last_value = new_value
    
    # Last value (Q69): ensure red or orange and at least 11
    if np.random.choice([True, False], p=[0.75, 0.25]):  # Red: 75% chance
        max_val = int(last_value * 0.93)
        new_value = np.random.randint(11, max_val) if max_val > 11 else 11
    else:  # Orange: 25% chance
        min_val = int(last_value * 0.93)
        new_value = np.random.randint(max(min_val, 11), last_value + 1)
    data.append(new_value)
...
def generate_plot_json(data):
    ...
    fig.update_layout(title="Random Data Bar Chart (Last 8 Mainly Red or Orange, Last Value > 10)")
