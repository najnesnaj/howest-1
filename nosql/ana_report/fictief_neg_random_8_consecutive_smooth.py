...
def generate_random_data():
    ...
    # Generate first 61 values with ±14% constraint
    for i in range(60):  # Generate up to index 60 (61 values total)
        last_value = data[-1]
        # Calculate bounds: ±14% of last_value, capped at 0 and 100
        min_val = max(1, int(last_value * 0.86))  # At least 1, -14%
        max_val = min(100, int(last_value * 1.14))  # At most 100, +14%
    ...
    # Generate remaining values, applying red/orange constraints for the 8 consecutive bars
    for i in range(61, 69):  # Generate indices 61 to 68
        last_value = data[i - 1]
        # Calculate ±14% bounds, capped at 0 and 100
        min_val_14 = max(1, int(last_value * 0.86))  # At least 1, -14%
        max_val_14 = min(100, int(last_value * 1.14))  # At most 100, +14%
        
        if start_idx <= i < start_idx + 8:  # In the 8 consecutive red/orange bars
            if np.random.choice([True, False], p=[0.5, 0.5]):  # Red: 50% chance
                # Red: new_value < last_value * 0.93 for < -7% change, non-zero
                max_val = min(int(last_value * 0.93), max_val_14)  # Respect -7% and -14%
                ...
                min_val = max(int(last_value * 0.93), 1)  # At least 1
                max_val = min(last_value, max_val_14)  # Respect last_value and +14%
                new_value = np.random.randint(min_val, max_val + 1) if min_val <= max_val else last_value
        else:  # Outside the 8 consecutive bars
...
def generate_plot_json(data):
    ...
    fig.update_layout(title="Random Data Bar Chart (8 Consecutive Red or Orange, Non-Zero, ±14% Change)")
